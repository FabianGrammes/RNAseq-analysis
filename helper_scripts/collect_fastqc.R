...
zips <- list.files(qc.folder, pattern ='.zip')
...
# Loop aver all unzipt files:
qcs <- list.files(path = file.path(qc.folder, "tmp" ), full.names = TRUE)
lv <- vector('list', length(zips))
...
# Separarte R1 and R2
r1 <- sep.reads(full.list, '_R1_')
r2 <- sep.reads(full.list, '_R2_')
